...
# Import training set
X_train = reduce_memory_usage(
    pd.read_csv(
        'data/X_train.csv',
        index_col='SK_ID_CURR')
)
X_train.replace({0: 'Non-defaulter', 1: 'Defaulter'}, inplace=True)
# Import testing sets
...
# General performance callback
@app.callback(
    Output('general-performance', 'figure'),
    [Input('feature', 'value')]
)
def update_general_performance(feat):
    fig = train_density_with_central_tendencies_by_target(feat, X_train)
